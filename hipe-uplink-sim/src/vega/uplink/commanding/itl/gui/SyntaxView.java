 ...
 * TokenMarker. This class should not be used directly; a SyntaxEditorKit should
 * be used instead.
 * 
 * @author Slava Pestov
 * @author Bruce Quig
...
public final class SyntaxView extends PlainView {
	...
	// --------------------------------------------------------------------------
	/**
	 * Paints the specified line. This method performs the following: - Gets the
	 * token marker and color table from the current document, typecast to a
	 * SyntaxDocument. - Tokenizes the required line by calling the markTokens()
	 * method of the token marker. - Paints each token, obtaining the color by
	 * looking up the the Token.id value in the color table. If either the
	 * document doesn't implement SyntaxDocument, or if the returned token
	 * marker is null, the line will be painted with no colorization. Currently,
	 * we assume that the whole document uses the same font. To support font
	 * changes, some of the code from "initilise" needs to be here to be done
	 * repeatedly for each line.
	 * 
	 * @param lineIndex
	 *            The line number
	 * @param g
	 ...
	 * @param x
	 *            The x co-ordinate where the line should be painted
	 * @param y
	 *            The y co-ordinate where the line should be painted
	 */
	public void drawLine(int lineIndex, Graphics g, int x, int y) {
		...
		try {
			...
			if (tokenMarker == null) {
				Utilities.drawTabbedText(line, x + BREAKPOINT_OFFSET, y, g,
						this, 0);
			} else {
	...
	public Rectangle lineToRect(Shape a, int aline) {
		...
		if (metrics != null) {
			...
			r = new Rectangle(alloc.x, alloc.y + (aline * metrics.getHeight()),
	 ...
	 * coordinate space of the view mapped to it. This is a redefined method
	 * from PlainView that adds an offset for the view to allow for a breakpoint
	 * area in the associated editor.
	 * 
	 * @param pos
	 ...
	 * 
	 * public void paint( Graphics g, Shape allocation) { // paint the lines
	 * super.paint(g, allocation); // paint the tag separator line
	 * g.setColor(Color.gray); Rectangle bounds = allocation.getBounds();
	 * g.drawLine( bounds.x + TAG_WIDTH, 0, bounds.x + TAG_WIDTH, bounds.y +
	 * bounds.height + 1); }
	 */
	/**
		...
		if (y < alloc.y) {
			...
			if (x < alloc.x) {
				...
				try {
					...
					int offs = p0
							+ Utilities.getTabbedTextOffset(buffer, metrics,
									tabBase, x, this, p0);
					return offs;
	...
	/**
	 * paints a line with syntax highlighting, redefined from
