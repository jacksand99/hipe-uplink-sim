package vega.hipe.products.gui;
...
*
*/
public class AbstractXmlMapContextXmlEditor extends AbstractVariableEditorComponent<AbstractXmlMapContext> {
	/**
	 * 
	 */
	...
	AbstractXmlMapContext data;
	XMLTextEditor editor;
	JScrollPane scroll;
	JSearchDialog searchDialog;
	JLabel classLabel;
	public AbstractXmlMapContextXmlEditor(){
	...
	public void init(){
		...
		classLabel=new JLabel(data.getFileName());
		...
		editor.setText(data.getXmlData());
		scroll=new JScrollPane(editor);
		scroll.setRowHeaderView(new TextLineNumber(editor));
		searchDialog=new JSearchDialog(editor,data.getFileName());
 ...
 protected List<String> getLines(String text) {
     ...
     try{
     	...
     	String text = editor.getText();
			...
			data.save();
					...
					JOptionPane.showMessageDialog(AbstractXmlMapContextXmlEditor.this,
						    e1.getMessage(),
						    "Error",
	...
	public void saveAs(){
     try{
	          ...
	          try{
	        	  chooser.setSelectedFile(new File(data.getPath()+"/"+data.getFileName()));
	          }catch (Exception e3){
	        	chooser.setCurrentDirectory(new File(System.getProperty("user.home")));
	          }
	          chooser.setMultiSelectionEnabled(false);
	          
	          int option = chooser.showSaveDialog(AbstractXmlMapContextXmlEditor.this);
	          //File sf;
	          if (option == JFileChooser.APPROVE_OPTION) {
	            File sf = chooser.getSelectedFile();
	            data.setFileName(sf.getName());
	            data.setPath(sf.getParent());

	            save();
	            classLabel.setText(data.getFileName());
 ...
 public boolean makeEditorContent() {
	...
	public Icon getComponentIcon() {
     ...
     try {
         URL resource = AbstractXmlMapContextXmlEditor.class.getResource(IconResources.HUS_ICON);
	...
	@Override
	protected Class<? extends AbstractXmlMapContext> getVariableType() {
		return AbstractXmlMapContext.class;
	}
