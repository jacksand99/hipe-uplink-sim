	...
	for k in tm:
		if (k>=sTime and k<=eTime):
			if (hm.getStates(k).getStateForMode(forbidden_status)==forbidden_status):
				simulationContext.log("Forbidden "+forbidden_status+" at "+dateFormat2.format(java.util.Date(k)))
...
forb_man=["RP_PW_ICA_HV","SR_NAC_Door_Open","SR_WAC_Door_Open", "ALICE_PW_On","ALICE_PW_Operational"]
forb_comm=["ACSS120Z","ACSS180Z","ACSS123Z","ACSS124Z"]
#times=SimulationContext.getInstance().historyModes.getTimes();
...
forb_man=["RN_PW_DPU_Only","RN_PW_COPS","RP_PW_ICA_HV","SR_NAC_Door_Open","SR_WAC_Door_Open", "ALICE_PW_On","ALICE_PW_Operational","VR_PW_ME_Idle","VR_PW_ME_Safe"]
forb_comm=["ACSS120Z","ACSS180Z","ACSS123Z","ACSS124Z"]
for j in simulationContext.getHistoryModes().getTimes():
	states=simulationContext.getHistoryModes().getStates(j+1)
	if (states==None):
